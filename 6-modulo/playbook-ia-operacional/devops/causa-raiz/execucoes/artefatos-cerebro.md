# Execução — Degradação de busca/indexação no Cerebro

**Modelo:** Claude Opus 5 (`claude-opus-5`) via claude.ai · temperatura 0.2 · sem histórico prévio · pacote de artefatos inserido nos três parâmetros.

## Entrada

### Artefato 1 — `{{CONFIG_CLUSTER}}` (cerebro.yaml, repositório de infra)
...
### Artefato 2 — `{{SERIE_METRICAS}}` (série de métricas do dashboard do Sentinel)
...
### Artefato 3 — `{{TRECHO_LOGS}}` (log nativo do Elasticsearch, cerebro-node-3)
...
## Saída do modelo (análise de causa-raiz)

...
description: Escreve e confere arquivos de manifesto Kubernetes contra o Padrão de Manifests da Metacortex — nomenclatura, rótulos, resiliência e segurança. Use sempre que houver um arquivo YAML em mãos ou a intenção de produzir um: criar, revisar ou corrigir manifesto de qualquer cliente do parque (Deployment, Service, Job, CronJob, ConfigMap, Secret), inclusive quando a pessoa apenas perguntar "esse manifesto está no padrão da casa?", pedir para empacotar uma aplicação para subir num cluster, quando o apply for rejeitado pela API, ou quando for corrigir no YAML um selector que não casa com os rótulos do pod depois que a triagem já apontou essa causa. Vale mesmo quando o pedido não cita o padrão. Não use para diagnosticar sintoma de workload que já está rodando, para infraestrutura fora do Kubernetes, nem para pergunta conceitual sem arquivo envolvido.
---
